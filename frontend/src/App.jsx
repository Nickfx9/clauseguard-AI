...
export default function App() {
  ...
  // ✅ IMPROVED: More human + product-readable reasoning flow
  const reasoningSteps = [
    "Understanding contract structure and key obligations...",
    "Identifying risky legal and financial clauses...",
    "Checking legal patterns against verified statutory sources...",
    "Scanning live Web IQ signals for known disputes and risks..."
  ];
  ...
  useEffect(() => {
    ...
    if (loading) {
      ...
      interval = setInterval(() => {
        setCurrentStep((prev) =>
          prev < reasoningSteps.length - 1 ? prev + 1 : prev
        );
      }, 1200);
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans selection:bg-indigo-500/30">
      ...
      {/* HEADER */}
      <header className="border-b border-slate-800 bg-slate-950/60 backdrop-blur sticky top-0 z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Multi-Step Contract Safety Engine powered by Microsoft Foundry IQ
            </p>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 space-y-4 shadow-lg">

            <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-300 border-b border-slate-800/60 pb-2">
              Contract Ingestion Terminal
            </h2>
            ...
            <form onSubmit={handleAudit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1.5">
                  Platform / Counterparty Identity
                </label>
              ...
              <button
                ...
                className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-500 text-white text-sm font-medium py-2.5 px-4 rounded-lg transition-all shadow-lg"
              >
                {loading ? 'Analyzing Contract...' : 'Analyze Contract Safety'}
              </button>
        ...
        {/* OUTPUT PANEL */}
        <section className="lg:col-span-7 bg-slate-950/60 border border-slate-800 rounded-xl p-6 min-h-[500px] flex flex-col shadow-lg">

          <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-300 border-b border-slate-800/60 pb-2 mb-4">
            ...
            <div className="flex-1 flex flex-col justify-center items-center space-y-6 py-12">
              ...
              <div className="space-y-2 text-center max-w-sm">
                ...
                <p className="text-xs text-indigo-400 animate-pulse font-medium">
                  AI Agent Working: {reasoningSteps[currentStep]}
                </p>
                ...
                <div className="w-48 h-1 bg-slate-800 mx-auto rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${((currentStep + 1) / reasoningSteps.length) * 100}%`
                    }}
                  />
                </div>
                ...
                <div className="pt-5 border-t border-slate-800/80">
                  ...
                  <ul className="space-y-2">
                    {result.citations.map((citation, idx) => (
                      <li
                        key={idx}
                        className="text-xs text-slate-300 bg-slate-900 border border-slate-800 rounded-lg p-2.5 font-mono"
                      >
                  ...
                  <div className="grid gap-2.5">
                    ...
                    {result.liveWebSignals.map((signal, idx) => (
                      <a
                        ...
                        className="block bg-slate-900 border border-slate-800 hover:border-amber-500/40 rounded-lg p-3 transition-all"
                      >
          ...
          {!loading && !result && (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <div className="w-12 h-12 rounded-full border border-dashed border-slate-800 flex items-center justify-center mb-3 text-slate-500">
                📋
              </div>
              ...
              <p className="text-slate-600 text-xs mt-1 max-w-sm">
                Paste a contract on the left to begin AI-powered safety analysis.
              </p>
